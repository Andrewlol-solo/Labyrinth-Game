...
# Класс, отвечающий за приведение призраков
class Ghost:
    def __init__(self, x_coord, y_coord, target, speed, img, direct, dead, box, id):
        ...
        self.rect = self.draw()
# Функция для отображения призрака
    def draw(self):
...
# Функция для передвижения желтого призрака
...
# Функция для передвижения красного призрака
    def move_red(self):
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
        ...
        if self.x_pos < -30:
            ...
            self.x_pos -= 30
        return self.x_pos, self.y_pos, self.direction
# Функция для передвижения синего призрака
    def move_blue(self):
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
        ...
        if self.x_pos < -30:
            ...
            self.x_pos -= 30
        return self.x_pos, self.y_pos, self.direction
# Функция для передвижения зеленого призрака
    def move_green(self):
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
...
# Отображение бонуса, если он активен
    if powerup:
...
# Вычисление позиций игрока в сетке уровня
    num1 = (HEIGHT - 50) // 32
    num2 = WIDTH // 30
# Проверка на попадание в точки
    if 0 < player_x < 870:
...
def draw_player():
# Отрисовка игрока в зависимости от направления движения
    if direction == 0:
...
def check_position(centerx, centery):
    ...
    num2 = (WIDTH // 30)
    num3 = 15
# Проверка возможности поворота в текущем положении игрока
    if centerx // 30 < 29:
        if direction == 0:
        ...
        if direction == 3:
            if level[(centery - num3) // num1][centerx // num2] < 3:
                turns[2] = True
# Проверка на возможность поворота в зависимости от направления и позиции
...
# Движение игрока в зависимости от направления и допустимости поворота
...
# Обновление цели для привидений, чтобы двигаться в сторону игрока
